...
while (<IN>)
{
    ...
    foreach my $ind (0..$#{$alleles})
    {
        if (!defined($hetero->[$ind]))
        {
            $hetero->[$ind] = 0;
        }
        if (!defined($homo->[$ind]))
        ...
        if ($alleles->[$ind][0] eq $alleles->[$ind][1])
        {
            $homo->[$ind]++;
            next;
        }
...
print "individual\thomo\thetero\tratio\n";
foreach my $ind (0..$#{$homo})
{
    print "$ind\t";
    print $homo->[$ind];
    print "\t";
