...
class FlowcellStatisticsReport(QsubProcess):
    """
    Object to keep track of samples that come out of a flowcell
    from a sequencing run.  This object automatically sends reports
    when 1, 4, 16, and 64 samples have completed.
    """
    def __init__(self,config,key=int(-1),input_dir=None,base_output_dir=None,output_dir=None,date=strftime("%Y%m%d",localtime()),time=strftime("%H:%M:%S",localtime()),process_name='sample_qsub',complete_file=None,**kwargs):
        if sample is None:
