...
class GenericProcess(NumberedObject):
    ...
    def __init__(self,config,key=int(-1),sample=None,date=strftime("%Y%m%d",localtime()),time=strftime("%H:%M:%S",localtime()),process_name='generic',**kwargs):
        if sample is None:
...
class QsubProcess(GenericProcess):
    ...
    def __init__(self,config,key=int(-1),sample=None,input_dir=None,base_output_dir=None,output_dir=None,date=strftime("%Y%m%d",localtime()),time=strftime("%H:%M:%S",localtime()),process_name='qsub',complete_file=None,**kwargs):
        if sample is None:
            sample = Sample(config,key="dummy_sample_key")
        if sample.__class__.__name__ != "Sample":
            raise Exception("Trying to start a qcpipeline process on a non-sample.")
        GenericProcess.__init__(self,config,key=key,sample=sample,date=date,time=time,process_name=process_name,**kwargs)
...
class QualityControlPipeline(GenericProcess):
    ...
    def __init__(self,config,key=int(-1),sample=None,flowcell=None,description=None,recipe=None,input_dir=None,base_output_dir=None,date=strftime("%Y%m%d",localtime()),time=strftime("%H:%M:%S",localtime()),process_name='qcpipeline',sequencing_run=None,running_location='Space',storage_needed=500000000,**kwargs):
        ...
        GenericProcess.__init__(self,config,key=key,sample=sample,process_name=process_name,**kwargs)
